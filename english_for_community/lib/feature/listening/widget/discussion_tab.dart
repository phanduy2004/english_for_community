            ...
            children: [
              if (_replyingTo != null)
                Container(
                  ...
                  child: Row(children: [Expanded(child: Text(t.replyingToUser(_replyingTo!.userName), style: TextStyle(fontSize: AppTypography.mobileCaption, color: Colors.grey[600], fontWeight: FontWeight.w600))), InkWell(onTap: () => setState(() => _replyingTo = null), child: const Icon(Icons.close, size: 16, color: Colors.grey))]),
                ),
        ...
        Stack(clipBehavior: Clip.none, children: [
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(c.userName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: AppTypography.mobileBody)), const SizedBox(width: 8), Text(timeStr, style: const TextStyle(fontSize: AppTypography.mobileCaption, color: Colors.grey))]),
              const SizedBox(height: 4),
              RichText(text: TextSpan(style: const TextStyle(fontSize: AppTypography.mobileH2, fontFamily: 'Roboto', height: 1.4, color: Colors.black), children: _parseContentStyle(c.content))),
            ]),
          ),
          if (totalReactions > 0) Positioned(bottom: -10, right: 0, child: Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(AppRadius.card), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 2, offset: Offset(0, 1))]), child: Row(mainAxisSize: MainAxisSize.min, children: [...(displayReactions ?? []).map((t) => Padding(padding: const EdgeInsets.only(right: 2), child: _buildReactionIconWidget(t, size: 12))), Text("$totalReactions", style: const TextStyle(fontSize: AppTypography.mobileCaption, color: Colors.black54))]))),
        ]),
        Padding(padding: const EdgeInsets.only(left: 8, top: 6), child: Row(children: [GestureDetector(onTap: () => onReply(c), child: Text(t.discussionReply, style: const TextStyle(fontSize: AppTypography.mobileCaption, fontWeight: FontWeight.w600, color: Colors.grey))), const SizedBox(width: 16), Theme(data: Theme.of(context).copyWith(popupMenuTheme: const PopupMenuThemeData(color: Colors.white)), child: PopupMenuButton<ReactionType>(tooltip: t.discussionReactTooltip, offset: const Offset(0, -45), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.pill)), elevation: 4, child: Row(children: [myReaction != null ? _buildReactionIconWidget(myReaction, size: 14) : const Icon(Icons.favorite_border, size: 14, color: Colors.grey), const SizedBox(width: 4), Text(myReaction != null ? _getReactionLabel(myReaction, t) : t.reactionLike, style: TextStyle(fontSize: AppTypography.mobileCaption, fontWeight: FontWeight.w600, color: myReaction != null ? _getReactionColor(myReaction) : Colors.grey))]), itemBuilder: (context) => ReactionType.values.map((type) => PopupMenuItem(value: type, padding: const EdgeInsets.symmetric(horizontal: 8), height: 40, child: Center(child: _buildReactionIconWidget(type, size: 24)))).toList(), onSelected: (type) => onReact(c.id, type)))])),
      ])),
    ]);
  }
